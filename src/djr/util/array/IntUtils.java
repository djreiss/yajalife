...
/**
 * Template class <code>IntUtils</code>.
 *
 ...
 * rpl Integer Integer IntUtils.java ; rpl Int Int IntUtils.java ; rpl int int IntUtils.java
 * javac IntUtils.java
 *
...
public class IntUtils {
   ...
   public static final int Random( int max ) { 
      return (int) ( DoubleUtils.Random() * max ); }
   public static final int Random( double min, double max ) { 
      return (int) ( min + ( max - min ) * DoubleUtils.Random() ); }
   public static final int Random( int min, int max ) { 
      return Random( (double) min, (double) max ); }
   public static final int RandChoose( int lo, int hi ) {
      return (int) ( (long) lo + (long) ( ( 1L + (long) hi - (long) lo ) * 
					   DoubleUtils.Random() ) ); }
   public static final int RandChoose( int hi ) {
      return RandChoose( 0, hi ); }
   // END NO BOOL
   public static final int RandChoose( int[] in ) {
   ...
   // NO BOOL
   public static final int Normal( double mean, double stddev ) { 
      return (int) Normal.staticNextDouble( mean, stddev ); }
   public static final int Normal() { 
      return Normal( 0.0, 1.0 ); }

   public static final double Log( int in ) { return in <= 0 ? -999 : Math.log( in ); }
   public static final double Log2( int in ) { return DoubleUtils.Log( in ) / LOG2; }
   public static final double Log10( int in ) { return DoubleUtils.Log( in ) / LOG10; }
   public static final double Sign( int in ) { return in < 0 ? -1.0 : 1.0; }
   // END NO BOOL

   public static final int[] New( int nx ) { 
      return new int[ nx ]; }
   public static final int[][] New( int nx, int ny ) { 
      return new int[ nx ][ ny ]; }
   public static final int[][][] New( int nx, int ny, int nz ) { 
      return new int[ nx ][ ny ][ nz ]; }
   public static final int[][][][] New( int nx, int ny, int nz, int na ) { 
      return new int[ nx ][ ny ][ nz ][ na ]; }

   public static final int[] Resize( int[] in, int newx ) {
      int[] out = New( newx ); Copy( out, in ); return out; }
   public static final int[][] Resize( int[][] in, int newx, int newy ) {
      int[][] out = New( newx, newy ); Copy( out, in ); return out; }
   public static final int[][][] Resize( int[][][] in, int newx, int newy, int newz ) {
      int[][][] out = New( newx, newy, newz ); Copy( out, in ); return out; }
   public static final int[][][][] Resize( int[][][][] in, int newx, int newy, int newz, int newa ) {
      int[][][][] out = New( newx, newy, newz, newa ); Copy( out, in ); return out; }

   public static final int[] New( int[] in ) {
      if ( in == null ) return null;
      int[] out = New( in.length ); Copy( out, in ); return out; }
   public static final int[][] New( int[][] in ) {
   ...
   public static final Object NewObj( Object obj ) {
      if ( obj instanceof int[] ) return New( (int[]) obj );
      else if ( obj instanceof int[][] ) return New( (int[][]) obj );
      else if ( obj instanceof int[][][] ) return New( (int[][][]) obj );
      return null; }

   public static final int[] SubArr( int[] in, int start, int end ) {
      int[] out = New( end - start + 1 );
      System.arraycopy( in, start, out, 0, end - start + 1 );
      return out; }

   // NO BOOL
   public static final int[] Sequence( int length ) {
      int[] out = New( length ); for ( int i = 0; i < length; i ++ ) out[ i ] = (int) i;
      return out; }
   ...
   public static final void Print( Object in ) {
      if ( in instanceof int[] ) Print( (int[]) in, 0, ( (int[]) in ).length ); 
   ...
   public static final void Print( int[] in, int start, int end ) {
      if ( in == null ) System.out.println( "NULL" );
      for ( int i = start; i < end; i ++ ) System.out.print( in[ i ] + " " );
      System.out.println(); }

   // NO BOOL
   public static final void Printf( String fmt, int in ) {
      if ( Double.isInfinite( (double) in ) ) in = - Integer.MAX_VALUE;
      Format.print( System.out, fmt, in ); };
   public static final String SPrintf( String fmt, int in ) {
      if ( Double.isInfinite( (double) in ) ) in = - Integer.MAX_VALUE;
      return ( new Format( fmt ) ).form( in ); }

   public static final void PrintCols( String fmt, int in1[], int in2[] ) {
      PrintCols( fmt, new int[][]{ in1, in2 } ); }
   public static final void PrintCols( String fmt, int in[][], boolean lines ) {
	 ...
	 System.out.println(); } }
   public static final void PrintCols( String fmt, int in[][] ) {
      PrintCols( fmt, in, false ); }

   public static final void Printf( String fmt, Object in ) {
      if ( in == null ) System.out.println( "NULL" );
      if ( in instanceof int[] ) { for ( int i = 0, size = ( (int[]) in ).length; i < size; i ++ )
	 Printf( fmt, ( (int[]) in )[ i ] ); }
      else { for ( int i = 0, size = ( (Object[]) in ).length; i < size; i ++ )
   ...
   public static final String SPrintf( String fmt, Object in ) {
      ...
      String out = "";
      if ( in instanceof int[] ) { for ( int i = 0, size = ( (int[]) in ).length; i < size; i ++ )
	 out += SPrintf( fmt, ( (int[]) in )[ i ] ); }
      else { for ( int i = 0, size = ( (Object[]) in ).length; i < size; i ++ )
	 out += SPrintf( fmt, ( (Object[]) in )[ i ] ); }
      return out; }

   public static final void PrintT( String fmt, int[][] in ) {
      for ( int y = 0, size = in[0].length; y < size; y ++ ) {
	 for ( int x = 0, s = in.length; x < s; x ++ ) Format.print( System.out, fmt, in[ x ][ y ] );
	 System.out.println(); } }
   public static final void PrintT( int[][] in ) {
      for ( int y = 0, size = in[0].length; y < size; y ++ ) {
	 for ( int x = 0, s = in.length; x < s; x ++ ) System.out.print( in[ x ][ y ] + " " );
	 System.out.println(); } }
   // END NO BOOL

   public static final int[][] Transpose( int[][] in ) {
      int out[][] = New( in[ 0 ].length, in.length );
   ...
   public static final void Copy( Object dest, Object src ) {
      ...
      if ( src instanceof int[] ) System.arraycopy( (int[]) src, 0, (int[]) dest, 0, 
			IntUtils.Min( ( (int[]) src ).length, ( (int[]) dest ).length ) );
   ...
   public static final int[] Flatten( int[][] in, int[] out ) {
      if ( out == null || out.length < NElem( in ) ) out = new int[ NElem( in ) ]; 
   ...
   private static int[][] UnflattenPriv( int in[], int start, int nx, int ny, int out[][] ) {
      if ( out == null || out.length < nx ) out = new int[ nx ][]; int ind = start;
      for ( int i = 0; i < nx; i ++ ) {
	 if ( out[ i ] == null || out[ i ].length < ny ) out[ i ] = new int[ ny ];
	 System.arraycopy( in, ind, out[ i ], 0, ny ); ind += ny;
      } return out; }
   public static int[][] Unflatten( int in[], int nx, int ny, int out[][] ) {
      return UnflattenPriv( in, 0, nx, ny, out ); }
   public static int[][] Unflatten( int in[], int nx, int ny ) {
      return UnflattenPriv( in, 0, nx, ny, null ); }

   public static int[][][] Unflatten( int in[], int nx, int ny, int nz, int out[][][] ) {
      if ( out == null || out.length < nx ) out = new int[ nx ][][]; int ind = 0, step = ny *nz;
      for ( int i = 0; i < nx; i ++ ) { out[ i ] = UnflattenPriv( in, ind, ny, nz, out[ i ] );
      ind += step; } return out; }
   public static int[][][] Unflatten( int in[], int nx, int ny, int nz ) {
      return Unflatten( in, nx, ny, nz, null ); }

   public static final void Zero( Object in, int start, int end ) {
      if ( in == null ) return;
      if ( in instanceof int[] ) Arrays.fill( (int[]) in, start, end, (int) 0 );
      else for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Zero( ( (Object[]) in )[ i ], start, end ); }
   public static final void Zero( Object in ) {
      if ( in == null ) return;
      if ( in instanceof int[] ) Arrays.fill( (int[]) in, (int) 0 );
      else for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Zero( ( (Object[]) in )[ i ] ); }
   public static final void Zero( int in[], int inds[] ) {
      if ( in == null ) return;
      else for ( int i = 0, s = inds.length; i < s; i ++ ) in[ inds[ i ] ] = 0;
   }

   public static final void Set( Object in, int from, int to, int val ) {
      if ( in instanceof int[] ) Arrays.fill( (int[]) in, from, to, val );
      else for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Set( ( (Object[]) in )[ i ], from, to, val ); }
   public static final void Set( Object in, int val ) {
      if ( in == null ) return;
      if ( in instanceof int[] ) Arrays.fill( (int[]) in, val );
      else for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Set( ( (Object[]) in )[ i ], val ); }
   public static final void Set( int in[], int inds[], int val ) {
      if ( in == null ) return;
   ...
   public static final int Sum( Object in, int min, int max ) {
   // END BOOL ONLY */
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
	 if ( max >= inn.length ) max = inn.length - 1;
   // NO BOOL
	 int out = 0; for ( int i = min; i <= max; i ++ ) out += inn[ i ]; return out; 
   // END NO BOOL
   /* BOOL ONLY
   ...
   public static final int Sum( Object in ) {
   // END BOOL ONLY */
      if ( in instanceof int[] ) return Sum( (int[]) in, 0, ( (int[]) in ).length ); 
      else {
   // NO BOOL
	 int out = 0; 
   // END NO BOOL
   ...
   public static final int SumSquared( Object in ) {
      if ( in == null ) return 0;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; int out = 0; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) out += inn[ i ] * inn[ i ]; return out;
      } else {
	 int out = 0; for ( int i = 0, s = ( (Object[]) in).length; i < s; i ++ ) 
	    out += SumSquared( ( (Object[]) in )[ i ] ); return out; }
   }

   public static final int SumSquared2( Object in ) {
      if ( in == null ) return 0;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; int out = 0; 
   ...
   public static final int Prod( Object in ) {
      if ( in == null ) return 0;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; int out = inn[ 0 ]; for ( int i = 1, s = inn.length; i < s; i ++ ) out *= inn[ i ]; return out;
      } else {
	 int out = Prod( ( (Object[]) in )[ 0 ] ); for ( int i = 1, s = ( (Object[]) in).length; i < s; i ++ ) 
	    out *= Prod( ( (Object[]) in )[ i ] ); return out; }
   }

   public static final int Max( Object in ) { int out = - Integer.MAX_VALUE;
      if ( in == null ) return 0;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static final int Min( Object in ) { int out = Integer.MAX_VALUE;
      if ( in == null ) return 0;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static final int[] MinMax( Object in, int inout[] ) { 
      if ( in == null ) return null;
      if ( inout == null ) { inout = New( 2 ); 
      inout[ 0 ] = Integer.MAX_VALUE; inout[ 1 ] = - Integer.MAX_VALUE; }
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static final int[] MinMax( Object in ) { 
      return MinMax( in, null ); }

   public static final void Round( Object in ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) (int) inn[ i ]; }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Round( ( (Object[]) in )[ i ] ); }
   }

   public static final void Floor( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static final void Ceil( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) if ( inn[ i ] > val ) inn[ i ] = val; }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Mult( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static final void Mult( Object in, Object in2 ) { 
      ...
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] *= ( (int[]) in2 )[ i ]; }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Divide( int val, Object in ) { 
      ...
      if ( val == 0 ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) ( val / inn[ i ] ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Divide( ( (Object[]) in )[ i ], val ); }
   }
   ...
   public static final void Add( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] += val; }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Sub( Object in, int val ) { 
      Add( in, (int) -val ); }
   ...
   public static final void Normal( Object in, double mean, double stddev ) {
      ...
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) nrand.nextDouble(); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Log10( Object in ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) Log10( inn[ i ] ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Log10( ( (Object[]) in )[ i ] ); }
   }
   ...
   public static final void Log2( Object in ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) Log2( inn[ i ] ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Log2( ( (Object[]) in )[ i ] ); }
   }
   ...
   public static final void Log( Object in ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) inn[ i ] = (int) Log( inn[ i ] ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Log( ( (Object[]) in )[ i ] ); }
   }

   public static final void Pow( int val, Object in ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) 
	 inn[ i ] = (int) Math.pow( val, (double) inn[ i ] ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Pow( ( (Object[]) in )[ i ], val ); }
   }

   public static final void Pow( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) 
	 inn[ i ] = (int) Math.pow( (double) inn[ i ], val ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Pow( ( (Object[]) in )[ i ], val ); }
   }

   public static final void Pow2( Object in, int val ) { 
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) 
	 inn[ i ] = (int) ( Sign( inn[ i ] ) * Math.pow( (double) inn[ i ], val ) ); }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Abs( Object in ) { 
      ...
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) {
	 int x = inn[ i ]; inn[ i ] = (int) ( x > 0 ? x : -x ); } }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
	 Abs( ( (Object[]) in )[ i ] ); }
   }

   public static final void Replace( Object in, int from, int to ) {
      if ( in == null ) return;
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) 
   ...
   public static final int WhereMax( int[] in, int maxind ) { 
      if ( in == null ) return -1;
      int out = - Integer.MAX_VALUE ; int ind = -1;
      maxind = Math.min( in.length, maxind+1 );
      for ( int i = 0; i < maxind; i ++ ) 
	 if ( out < in[ i ] ) { out = in[ i ]; ind = i; } return ind; }
   public static final int WhereMax( int[] in ) {
      return WhereMax( in, in.length ); }

   public static final int WhereMin( int[] in, int maxind ) { 
      int out = Integer.MAX_VALUE ; int ind = -1;
      maxind = Math.min( in.length, maxind+1 );
      for ( int i = 0; i < maxind; i ++ ) if ( out > in[ i ] ) { out = in[ i ]; ind = i; } 
      return ind; }
   public static final int WhereMin( int[] in ) { 
      return WhereMin( in, in.length ); }

   public static final void MaxNorm( Object in ) { 
      int max = Max( in ); Divide( in, max ); }
   public static final void UnitNorm( Object in ) {
      int sum = Sum( in ); Divide( in, sum ); }

   public static final double Integral( int[] inx, int[] iny ) { // trapezoid rule
      double sum = 0.0;
   ...
   public static final int[] Integrate( int[] in, int[] out ) {
   ...
   public static final boolean Equals( Object in1, Object in2 ) {
      if ( in1 instanceof int[] ) return Arrays.equals( (int[]) in1, (int[]) in2 );
      else {
   ...
   public static int NEqualTo( Object in, int val ) { 
      int out = 0; 
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static int NNotEqualTo( Object in, int val ) { 
      return NElem( in ) - NEqualTo( in, val ); }

   public static boolean AnyEqualTo( Object in, int val ) { 
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
   ...
   public static boolean AnyNotEqualTo( Object in, int val ) { 
      if ( in instanceof int[] ) { int[] inn = (int[]) in; 
      for ( int i = 0, s = inn.length; i < s; i ++ ) if ( inn[ i ] != val ) return true; }
      else { for ( int i = 0, s = ( (Object[]) in ).length; i < s; i ++ ) 
   ...
   public static final void Mask( int[] in, boolean[] flags, boolean val ) {
      for ( int i = 0, s = in.length; i < s; i ++ ) if ( flags[ i ] == val ) in[ i ] = 0; }
   public static final int IndexOf( int[] in, int val ) {
      for ( int i = 0, s = in.length; i < s; i ++ ) if ( in[ i ] == val ) return i; return -1; }
   public static final boolean Contains( int[] in, int val ) {
      return ( IndexOf( in, val ) != -1 ); }
   public static final int LastIndexOf( int[] in, int val ) {
      for ( int i = in.length - 1; i >= 0; i -- ) if ( in[ i ] == val ) return i; return -1; }
   public static final int[] Index( int in[], int index[] ) {
      int out[] = New( in.length ); 
      for ( int i = 0, s = in.length; i < s; i ++ ) out[ i ] = in[ index[ i ] ]; return out; }

   public static final int[] Reverse( int[] in ) {
      for ( int i = 0, s = in.length; i < s/2; i ++ ) {
	 int temp = in[ i ]; in[ i ] = in[ s-i-1 ]; in[ s-i-1 ] = temp; }
      return in; }

   // NO BOOL
   public static final void Sort( int[] in ) { Arrays.sort( in ); }
   public static final int[] Indexx( int[] in, int[] out ) {
      double darr[] = Convert( in );
      try { return djr.nr.Indexx.indexx( darr, out ); } catch( djr.nr.NRException e ) { }; 
      return null; }
   public static final int[] IRank( int[] in, int[] out ) {
      double darr[] = Convert( in );
      try { return djr.nr.Indexx.irank( darr, out ); } catch( djr.nr.NRException e ) { }; 
      return null; }

   public static final int Search( int[] in, int val, boolean sort ) {
      if ( sort ) Sort( in ); return Arrays.binarySearch( in, val ); }
   public static final int Search( int[] in, int val ) {
      return Search( in, val, true ); }

   public static final int[] Uniq( int[] in ) {
      IntVector v = new IntVector();
      for ( int i = 0, s = in.length; i < s; i ++ ) 
	 if ( ! v.contains( in[ i ] ) ) v.addElement( (int) i );
      return v.data();
   }
   // END NO BOOL

   public static final int NElem( Object in ) { 
      if ( in instanceof int[] ) return ( (int[]) in ).length; 
   ...
   public static final double Median( int[] in ) {
      final int nx = in.length; int temp[] = New( nx );
      Copy( temp, in ); Sort( temp ); double out = (double) temp[ (nx+1)/2 ];
      if ( nx % 2 != 1 ) out = ( (double) temp[ nx/2 ] + (double) temp[ nx/2+1 ] ) / 2.0;
      return out; }

   public static final double Variance( int[] in, double mean ) {
      double sum = 0.0; int nelem = 0;
      for ( int i = 0, s = in.length; i < s; i ++ ) 
	 sum += ( in[ i ] - mean ) * ( in[ i ] - mean );
      return sum / ( (double) in.length - 1.0 ); }

   public static final double Stddev( int[] in, double mean ) {
      return Math.sqrt( Variance( in, mean ) ); }

   public static final double Stddev( int[] in ) {
      return Stddev( in, Mean( in ) ); }

   public static final void Stats( int[] in, double[] out ) {
      out[ 0 ] = Mean( in ); out[ 1 ] = Median( in ); out[ 2 ] = Stddev( in ); }
   public static final double[] Stats( int[] in ) {
      double out[] = new double[ 3 ]; Stats( in, out ); return out; }

   public static void PrintStats( int[] data ) {
   ...
   public static final double[][] Histogram( int[] in, int nbins ) {
   ...
   public static final double[] Histogram( int[] in, int[] bins ) {
   ...
   public static final double[] Convolve( int x[], int kern[], double out[] ) {
   ...
   public static final double logGamma( int xx ) {
      return cern.jet.stat.Gamma.logGamma( (double) xx );
   }
   // END NO BOOL

   public static final int[] FTokenize( String str, final String tok ) { 
      return FTokenize( MyUtils.Tokenize( str, tok ) ); }

   public static final int[] FTokenize( String str[] ) { 
      return FTokenize( str, 0, str.length ); }

   public static final int[] FTokenize( String str[], int start, int end ) { 
      int[] out = New( end - start );
      for ( int i = start; i < end; i ++ ) {
   // NO BOOL
	 try { out[ i - start ] = (int) Integer.parseInt( str[ i ] ) ; } 
   // END NO BOOL
   /* BOOL ONLY
	 try { out[ i - start ] = Integer.valueOf( str[ i ] ).IntegerValue() ; } 
   // END BOOL ONLY */
	 catch( Exception e ) { out[ i ] = 0; } } return out; }

   public static final int[] FromVector( Vector v ) {
      if ( v == null || v.size() <= 0 ) return null;
      int out[] = New( v.size() );
      try {
	 for ( int i = 0, s = v.size(); i < s; i ++ )
	    out[ i ] = ( (Integer) v.elementAt( i ) ).intValue();
      } catch( Exception e ) { };
      return out;
   }

   public static final Vector ToVector( int[] in ) {
      if ( in == null ) return null;
      Vector v = new Vector();
      for ( int i = 0, s = in.length; i < s; i ++ ) v.addElement( new Integer ( in[ i ] ) );
      return v;
   }
}
