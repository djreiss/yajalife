...
public class Genome extends Griddable implements Constants, Serializable {
   ...
   private void initPrivate( final Globals g, String fileName ) {
      this.owner = this;
      this.dead = false;
      this.globals = g;
      if ( dictionary == null ) {
   ...
   public final void draw( Graphics g, int xx, int yy, int cs ) {
   ...
   public final void step( final long numSteps ) {
	 ...
	 cur = wrapPtr( cur );
	 int gen[] = owner.genes;
	 final int cmd = gen[ cur ]; // Get the command
   ...
   protected final int wrapPtr( int ptr ) {
      final int len = owner.genes.length;
      if ( ptr >= len ) ptr %= len; 
   ...
   protected final int getComplementCommand( final int cmd ) {
   ...
   protected final int getComplementIndex( int r ) { 
      int x = r + 1;
      if ( x >= reg.length ) x = 0;
      return x;
   }
   ...
   protected final int getLabelAt( int where, int[] ls ) {
   ...
   protected final int getInstructionSize( int where ) {
   ...
   protected final int getPrecedingInstructionSize( int where ) {
      int i = 1, gen[] = owner.genes;
      where = wrapPtr( -- where );
   ...
   protected final void skipCmd( int numCmds ) {
      int gen[] = owner.genes;
   ...
   protected final int searchForComplementLabels( final boolean forward, final int[] labs,
						  final int nlabs ) {
      if ( nlabs <= 0 ) return 0;
      int ptr = cur, count = 0;
      boolean found = false;
      final int gen[] = owner.genes, len = gen.length;
      if ( compLabels == null ) compLabels = new int[ globals.MAX_LABELS ];
      final int[] comps = compLabels;
   ...
   protected boolean express( final int cmd, final int[] labs, final int lsize ) {
   ...
   protected final void doShift( boolean right, final int Rind ) {
   ...
   protected final void doRand( final int Rind ) {
      reg[ Rind ] = randomCmd();
   }

   // Zero the current register
   protected final void doZero( final int Rind ) {
      if ( ! negated ) reg[ Rind ] = 0;
      else reg[ Rind ] = Integer.MAX_VALUE;
   }

   // Set the first bit of the current register to 1
   protected final void doBitOne( final int Rind ) {
   ...
   protected final void doNand( boolean nand, final int Rind ) {
   ...
   protected final void doNor( boolean nor, final int Rind ) {
      if ( negated ) nor = ! nor;
   ...
   protected final void doXor( boolean xor, final int Rind ) {
      if ( negated ) xor = ! xor;
   ...
   protected final void doNot( final int Rind ) {
      if ( ! negated ) reg[ Rind ] = ~reg[ Rind ];
      else reg[ Rind ] = ~reg[ getComplementIndex( Rind ) ];
   }

   // Increment the current register by the value in its complement
   protected final void doAdd( boolean add, final int Rind ) {
      if ( negated ) add = ! add;
   ...
   protected final void doDup( final int Rind ) {
   ...
   protected final void doOrder( final int Rind ) {
   ...
   protected final boolean isNotZero( final int Rind ) {
      if ( ! negated ) return reg[ Rind ] != 0;
      return reg[ Rind ] == 0;
   }

   // Is the current register greater than its complement?
   protected final boolean isGreater( final int Rind ) {
   ...
   protected final boolean isNotEqual( final int Rind ) {
      if ( ! negated ) return reg[ Rind ] != reg[ getComplementIndex( Rind ) ];
      return reg[ Rind ] == reg[ getComplementIndex( Rind ) ];
   }

   // Is the current register's lowest bit 1?
   protected final boolean isBitOne( final int Rind ) {
   ...
   protected final void doJump( boolean forward ) {
   ...
   protected final void doPush( final int Rind ) {
      pushValue( reg[ Rind ] );
   }

   // Pop value from current stack into current register
   protected final void doPop( final int Rind ) {
      reg[ Rind ] = popValue();
   }
   ...
   protected final int getTernary( int[] labs, int lsize ) {
      int out = 0;
   ...
   protected final void doSetNum( int[] labs, int lsize ) {
   ...
   protected final void doSearch( boolean forward, int[] labs, int nlabs ) {
      if ( negated ) forward = ! forward;
   ...
   protected final boolean isNCopy() {
   ...
   // Turn (left or right)
   protected final void doTurn( boolean right ) {
   ...
   protected final void doMove( boolean forward ) {
   ...
   protected final void doPointMutate() {
   ...
   // (subtract) one.
   protected final void doGestate( final int Rind, int[] labs, int lsize ) {
   ...
   protected final void doAllocate( final int Rind ) {
   ...
   protected final int[] doSplitMemory( int[] labs, int nlabs ) throws OutOfMemoryError {
      ...
      final int ind = nlabs > 0 ? getRegIndexForNOP( labs[ 0 ] ) : reg_A;
      int where = reg[ ind ];
   ...
   protected final int[] insDel( int[] daughter ) {
   ...
   protected final void doDivide( int[] labs, int nlabs ) {
      try {
   ...
   // fix that!)
   protected final void placeDaughter( int[] newGenes ) {
   ...
   protected final void doInject( final int Rcur, int[] labs, int nlabs ) {
      ...
      this.cur = 0;
      this.owner = host;
      final int pos = searchForComplementLabels( true, labs, nlabs );
   ...
   protected final void injectCode( int[] code, Genome host, int where ) {
      if ( code == null || code.length <= 0 ) return;
      final int[] old = host.genes;
   ...
   protected final void doGet( final int[] labs, final int nlabs ) {
      int food = randomCmd(); // Just get a random command value, for now
      final int ind = nlabs > 0 ? getRegIndexForNOP( labs[ 0 ] ) : reg_C; // Put it in the register
      reg[ ind ] = food;
   ...
   protected final void doPut( final int Rcur ) {
   ...
   protected final void testOutputs() {
   ...
   protected final void doJumpP( final int Rcur, final int[] labs, final int nlabs ) {
      Genome host = getNeighborGenome();
      if ( host == null || host.dead || host.genes == null ) return;
      int pos = reg[ Rcur ];
   ...
   protected final Genome getNeighborGenome() {
   ...
   // Place it in ?AX?.
   protected final void doSense( final int color, final int labs[], final int nlabs ) {
      final Grid gg = globals.resGrid;
      int newx = x, newy = x;
      boolean forward = negated ? false : true;
      switch ( direction ) {
      case NORTH: newy += ( forward ? -1 : 1 ); break;
      ...
      final int ind = nlabs > 0 ? getRegIndexForNOP( labs[ 0 ] ) : reg_A; // Put it in the register
      reg[ ind ] = value;
   }

   // Adjust the copy mutation rate
   protected final void modCMut( final int Rind ) {
   ...
   protected final void doModColor( final int Rind ) {
   ...
   protected final void readFromFile( Globals gl, String fileName ) {
